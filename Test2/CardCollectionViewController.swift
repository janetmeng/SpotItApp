...
//update the symbols, also make sure to get a zip of all the symbol photos and add them to the image asset
 ...
 /*
    ...
    */
    ...
    extension CardCollectionViewController: SymbolProtocol{
        func didTapImageView(for cell: CardCollectionViewCell) {
            UIView.animate(withDuration: 0.4) {
                cell.symbolImageView.transform = CGAffineTransform(scaleX: 0.01, y: 0.01)
            } completion: { (_ finished: Bool) in
                UIView.animate(withDuration: 0.4) {
                    cell.symbolImageView.transform = CGAffineTransform.identity
                }
            }
        }
    }
